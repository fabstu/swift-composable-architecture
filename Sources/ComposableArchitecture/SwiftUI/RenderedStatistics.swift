...
public class RenderedStatistics {
  ...
  fileprivate func printCounts() {
    countersLock.withLock {
      ...
      for (key, differencesDict) in counters {
        ...
        print("""
            \(overallCount)x \(key.prefix) \(key.file)#\(key.line) <\(key.viewStateType), \(key.viewActionType), ...>
            """)
        ...
        for (difference, count) in differencesDict.differences {
          ...
          print("\(Self.differenceLineIndent)\(count)x \(indentedDifference)")
        }
        
        if hiddenInitialStateDiffCount != 0 {
          // Print combined hidden initial state diff count.
          print("\(Self.differenceLineIndent)\(hiddenInitialStateDiffCount)x (Initial state) [diffs hidden]")
        }
      }
  ...
  private func add(
    ...
    countersLock.withLock {
      ...
      let key = Key(prefix: prefix, file: String(describing: file), line: line, viewStateType: viewStateType, viewActionType: viewActionType)
...
fileprivate class PrinterRunner {
...
//                print("RunningCounter: \(runningCounter)")
    ...
    try? await Task.sleep(nanoseconds: NSEC_PER_SEC * 999999999)
